...
public class MetShower : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < numAstro; ++i)
        {
            ...
            mr.enabled = false;
            rd.AddForce(new Vector2(-0.2f, 0.5f), ForceMode2D.Impulse);
        }
    }
    ...
    void Update()
    {
        ...
        for (int i = 0; i < numAstro; ++i)
        {
            if (astroArray[i].transform.position.x <= 80f || astroArray[i].transform.position.y >= 120f)
            {
                Rigidbody2D rd;
                MeshRenderer mr;
                mr = astroArray[i].GetComponent<MeshRenderer>();
